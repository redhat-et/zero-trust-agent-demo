...
```shell
get_admin_token() {
  curl -s -X POST "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/realms/master/protocol/openid-connect/token" \
...
```shell
curl -X POST "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/admin/realms/spiffe-demo/clients" \
...
```shell
curl -s "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/admin/realms/spiffe-demo/clients" \
...
```shell
CLIENT_UUID=$(curl -s "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/admin/realms/spiffe-demo/clients?clientId=agent-service" \
...
```shell
curl -s "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/admin/realms/spiffe-demo/clients/$CLIENT_UUID/client-secret" \
...
Now the fun part - let's get a token and exchange it.
  ...
  AGENT_TOKEN=$(curl -s -X POST "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/realms/spiffe-demo/protocol/openid-connect/token" \
    -d "grant_type=client_credentials" \
  ...
  curl -s -X POST "https://keycloak-spiffe-demo.apps.ocp-beta-test.nerc.mghpcc.org/realms/spiffe-demo/protocol/openid-connect/token" \
