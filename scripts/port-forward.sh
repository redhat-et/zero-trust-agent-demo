...
# All infra services now listen on 8080 inside the cluster,
# so we map them to different local ports for CLI testing.
echo "Starting port forwards..."

kubectl -n spiffe-demo port-forward svc/web-dashboard 8080:8080 &
kubectl -n spiffe-demo port-forward svc/user-service 8082:8080 &
kubectl -n spiffe-demo port-forward svc/agent-service 8083:8080 &
kubectl -n spiffe-demo port-forward svc/document-service 8084:8080 &
kubectl -n spiffe-demo port-forward svc/opa-service 8085:8080 &
